...
// Use ONE embedding model everywhere — mixing models breaks cosine similarity.
...
// Promise that resolves once all knowledge base entries are embedded.
// Anything that reads embeddings should await this first.
let embeddingsReady: Promise<void> = Promise.resolve();

async function initializeEmbeddings() {
  for (const entry of knowledgeBase) {
    if (!entry.embedding) {
      try {
        entry.embedding = await embedText(entry.content);
      } catch (error) {
        console.error(`Failed to embed ${entry.category}`, error);
      }
    }
  }

  console.log("Knowledge embeddings ready.");
}

embeddingsReady = initializeEmbeddings();
...
- Pronouns such as he, him, his, and Gourav refer to Gourav Binoj unless another person is explicitly mentioned.
...
export async function searchKnowledgeBase(
  ...
  // Make sure the knowledge base is fully embedded before searching.
  ...
  try {
    ...
    console.error("[Gova AI] Failed to embed query", error);
    return [];
  }
...
export async function addKnowledgeEntry(
  ...
  try {
    ...
    console.error(`Failed to embed new entry (${category})`, error);
  }
...
// Replaces the knowledge base and re-embeds any entries missing embeddings.
