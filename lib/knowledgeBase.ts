...
// Load the precomputed embeddings cache (generated by scripts/generate-embeddings.ts)
// using fs.readFileSync instead of a JS `import ... from "./embeddings.json"` statement.
// A direct JSON import behaves inconsistently across CJS/ESM interop layers depending
// on the bundler/runtime (tsx, ts-node, Next.js webpack/Turbopack, etc.), which can
// silently produce an undefined binding. Reading the file manually sidesteps that
// entirely and works the same way everywhere.
function loadEmbeddingsCache(): Record<string, number[]> {
  try {
    ...
    console.warn(
      "[Gova AI] Could not load embeddings.json — knowledge base will have no cached embeddings until it's generated. " +
        "Run scripts/generate-embeddings.ts to create it."
    );
    return {};
  }
}

const embeddingsCache: Record<string, number[]> = loadEmbeddingsCache();
...
// Load precomputed embeddings from the cache file (generated offline via
// scripts/generate-embeddings.ts) instead of calling the API on every cold start.
for (const entry of knowledgeBase) {
  const cached = (embeddingsCache as Record<string, number[]>)[entry.id];
  if (cached) {
    entry.embedding = cached;
  } else {
    console.warn(
      `No cached embedding found for entry ${entry.id} (${entry.category}). ` +
        `Run scripts/generate-embeddings.ts to regenerate embeddings.json.`
    );
  }
}
...
// Wraps a Gemini API call with retry + exponential backoff for 429s,
// so a transient rate limit doesn't immediately surface as a hard failure.
async function withRetry<T>(fn: () => Promise<T>, retries = 3): Promise<T> {
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (status === 429 && !isLastAttempt) {
        ...
        console.warn(`[Gova AI] Rate limited (429), retrying in ${waitMs}ms...`);
        await sleep(waitMs);
        continue;
      }
      throw error;
  ...
  throw new Error("withRetry: exceeded retry attempts.");
}
...
`;
    ...
    if (status === 429) {
      console.error("[Gova AI Error] Rate limit / quota exceeded:", error);
